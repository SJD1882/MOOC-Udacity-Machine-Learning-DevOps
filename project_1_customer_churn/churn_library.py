...
def perform_feature_engineering(df: pd.DataFrame, response: str) -> pd.DataFrame:
    '''
    input:
              df: pandas dataframe
              response: string of response name [optional argument that could be used for naming variables or index y column]
...
def classification_report_image(y_train: pd.DataFrame,
    ...
    plt.text(0.01, 0.05, str(classification_report(y_test, ytest_preds_rf)),
             {'fontsize': 10}, fontproperties = 'monospace')
    plt.axis('off')
    plt.savefig(f'{output_folder}/random_forest_classification_report.png', bbox_inches='tight')
    plt.close()
    ...
    plt.text(0.01, 0.05, str(classification_report(y_test, ytest_preds_lr)),
             {'fontsize': 10}, fontproperties = 'monospace')
    plt.axis('off')
    plt.savefig(f'{output_folder}/logistic_reg_classification_report.png', bbox_inches='tight')
    plt.close()
...
def roc_curve_plot(y_test: pd.DataFrame,
   ...
   false_positive_lrc, true_positive_lrc, _ = roc_curve(y_test, y_test_preds_lr)
   false_positive_rfc, true_positive_rfc, _ = roc_curve(y_test, y_test_preds_rf)
...
if __name__ == '__main__':
    ...
    if len(command_line_arguments) == 2:
        ...
        _ = classification_report_image(y_train,
                                        y_test,
                                        y_train_preds_lr,
                                        y_train_preds_rf,
                                        y_test_preds_lr,
        ...
        _ = roc_curve_plot(y_test,
                           y_test_preds_lr,
                           y_test_preds_rf,
                           'images/results')
        
        _ = feature_importance_plot('models/rfc_model.pkl',
                                    X_test,
                                    'images/results')

    else:
        logger.error('Wrong number of arguments provided. Only requires data path.')
